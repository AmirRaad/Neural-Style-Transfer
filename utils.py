...
def prepare_img(img_path, target_shape, device):
    ...
    # normalize using ImageNet's mean because we are using VGG-19 which is a convolutional neural network
    # that is trained on more than a million images from the ImageNet database.

    # `ToTensor()` Converts a PIL Image or numpy.ndarray (H x W x C) in the range [0, 255]
    # to a torch.FloatTensor of shape (C x H x W) in the range [0.0, 1.0]
    # [0, 255] range worked much better than [0, 1] range (even though PyTorch models were trained on latter)
    ...
    # Normalization: image = (image - mean) / std, Renormalization: image = ((image * std) + mean)
...
def save_and_maybe_display(optimizing_img, dump_path, config, img_id, num_of_iterations, should_display=False):
    ...
    if img_id == num_of_iterations-1 or (saving_freq > 0 and img_id % saving_freq == 0):
        ...
        dump_img += np.array(IMAGENET_MEAN_255)
        print(dump_img.shape)
